...
export class PostListComponent implements OnInit {
  ...
  listOfColumns: any = [
    ...
    },
    {
      name: 'Category',
      sortOrder: null,
      sortFn: (a: PostType, b: PostType) =>
        a.categoriesPostId - b.categoriesPostId,
      sortDirections: ['ascend', 'descend', null],
      ...
      filterFn: (cateName: string, item: PostType) =>
        item.categoriesPost?.name.indexOf(cateName) !== -1,
  ...
  search(): void {
    ...
    this.listOfDisplayData = this.listOfData.filter(
      (item: PostType) => item.title.indexOf(this.searchValue) !== -1
    );
  }
}
