...
export class ProjectListComponent implements OnInit {
  ...
  listOfColumns: any = [
    ...
    },
    {
      name: 'Category',
      sortOrder: null,
      sortFn: (a: ProjectType, b: ProjectType) =>
        a.categoriesProjectId - b.categoriesProjectId,
      sortDirections: ['ascend', 'descend', null],
      ...
      filterFn: (cateName: string, item: ProjectType) =>
        item.categoriesProject?.name.indexOf(cateName) !== -1,
  ...
  search(): void {
    ...
    this.listOfDisplayData = this.listOfData.filter(
      (item: ProjectType) => item.name.indexOf(this.searchValue) !== -1
    );
  }
}
